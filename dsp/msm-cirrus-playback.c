...
static int cirrus_fb_port = AFE_PORT_ID_SECONDARY_TDM_TX;
static int cirrus_ff_port = AFE_PORT_ID_SECONDARY_TDM_RX;
...
static void msm_crus_check_calibration_value(void)
{
	...
	pr_debug("%s: left bound: Lower: %u Upper: %u , "
				"right bound Lower: %u Upper: %u", __func__,
	...
	if (crus_sp_cal_rslt.z_l > left_upper_bound ||
		...
		pr_err("%s: left calibartion %u over limit, "
				"set default value %u", __func__,
	...
	if (crus_sp_cal_rslt.z_r > right_upper_bound ||
		...
		pr_err("%s: right calibartion %u over limit, "
				"set default value %u", __func__,
...
temperature_left_show(struct device *dev, struct device_attribute *a, char *buf)
{
	...
	if ((out_cal0 != 2) || (out_cal1 != 2))
		return snprintf(buf, MAX_BUFSIZE, "Calibration is not done\n");

	r = buffer[3];
	t = (material * scale_factor * (r - z) / z) + (temp0 * scale_factor);

	return snprintf(buf, MAX_BUFSIZE,
...
resistance_left_show(struct device *dev, struct device_attribute *a, char *buf)
{
	...
	return snprintf(buf, MAX_BUFSIZE, "%d.%08d ohms\n", r / scale_factor,
		       r % scale_factor);
}
...
static int msm_crus_parse_spk(struct platform_device *pdev)
{
	swap_calibration =
		of_property_read_bool(
			pdev->dev.of_node, "cirrus,swap_calibration");

	if (of_property_read_u32(pdev->dev.of_node,
			"cirrus,top-speaker-impedance",
			&crus_cal.top_spk_impedance)) {
		dev_err(&pdev->dev,
				"parse cirrus,top-speaker-impedance failed\n");
		return -EINVAL;
	}
	...
	if (of_property_read_u32(pdev->dev.of_node,
			"cirrus,top-speaker-tolerance",
			&crus_cal.top_spk_tolerance)) {
		dev_err(&pdev->dev,
				"parse cirrus,top-speaker-tolerance failed\n");
		return -EINVAL;
	}
	...
	if (of_property_read_u32(pdev->dev.of_node,
			"cirrus,top-speaker-mean",
			&crus_cal.top_spk_mean)) {
		dev_err(&pdev->dev,
				"parse cirrus,top-speaker-mean failed\n");
		return -EINVAL;
	}
	...
	if (of_property_read_u32(pdev->dev.of_node,
		...
		dev_err(&pdev->dev,
				"parse cirrus,bottom-speaker-impedance failed\n");
		return -EINVAL;
	}
	...
	if (of_property_read_u32(pdev->dev.of_node,
			"cirrus,bottom-speaker-mean",
			&crus_cal.bottom_spk_mean)) {
		dev_err(&pdev->dev,
			"parse cirrus,bottom-speaker-mean failed\n");
		return -EINVAL;
	}
